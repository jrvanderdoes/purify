...
```{rm simple_resample_function}
set.seed(1234)
res <- resample_function(X = data_strat, fn = strat_function, type='stratify',
                         strata='z', size=200)
res$estimates
```
The resampling returns better estimates.
...
res <- resample(X = data_ts,type='sliding',blockSize = 5)
res1 <- resample(data_ts,type='segment',blockSize = 5)
```
