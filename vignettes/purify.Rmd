...
set.seed(1234)
res <- resample(data = data_strat, fn = strat_function,
                         strata='z', stratify_sizes='mean')
```
...
mse_function <- function(data) {
  pred <- unique(data$Class)[
    as.numeric(
      predict(glm(as.factor(Class) ~ ., data = data, 
                  family=binomial(link="logit")), 
              type="response")>0.5) + 1
  ]
...
results <- resample(data = credit, fn = mse_function, 
                             M = 2000, strata='Job')
summarize_resample(results)
mse_function(credit)
```
