...
two_group_tests <- function(data,
  ...
  if ("t" %in% tolower(tests)) {
    ...
    res <- append(res, list("ttest_student" = list(
      "pvalue" = t_test$p.value,
      "means" = t_test$estimate,
      "interval" = t_test$conf.int
    )))

    t_test1 <- stats::t.test(
      formula = form, data = data, var.equal = FALSE,
      conf.level = 1 - alpha
    )
    res <- append(res, list("ttest_welch" = list(
      "pvalue" = t_test1$p.value,
  ...
  if ("bayes" %in% tolower(tests)) {
    ...
    res <- append(res, list("bayes" = list(
      "pvalue" = bayes_t_test$p.value,
      "means" = bayes_t_test$estimate,
      "interval" = bayes_t_test$conf.int
    )))

    bayes_t_test1 <- Bolstad::bayes.t.test(
      formula = form, data = data,
      var.equal = FALSE, conf.level = 1 - alpha
    )
    res <- append(res, list(
      "bayes_welch" = list(
        "pvalue" = bayes_t_test1$p.value,
...
group_tests <- function(data,
                        tests = c(
                          "tukey", "snk", "lsd", "bt", "kramer", "duncan",
  ...
  if ("tukey" %in% tolower(tests)) {
    ...
    res <- append(res, list("tukey" = list(
      "details" = tukey_info,
      "groups" = tmp_group
    )))
  }
  ...
  if ("bt" %in% tolower(tests)) {
    ...
    tmp1 <- stats::pairwise.t.test(
      x = data[, 1], g = data[, 2], pool.sd = FALSE,
      p.adjust.method = "bonferroni"
    )
  ...
  if ("median" %in% tolower(tests)) {
    ...
    res <- append(res, list("median" = list(
      "pvalues" = tmp$p.value,
      "groups" =
        .PMCMRplus_summary(tmp, alpha = alpha)
    )))
