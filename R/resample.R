    ...
    if(data.vector){
      ...
      X_resampled <-
        sapply(1:M, function(i, n, X1, replace=replace, blocksize=blocksize,
          ...
          data_resample <- X1[sample(1:length(idxGroups), replace = replace,
                                     size = size_val),
                                ...
                                c(return_idxs,
                                  group_sizes_tmp[j-1] +
                                    unlist(sample(idxGroups[[j-1]],
                                                  size=sizes$Freq[j-1],
                                                  replace = replace))[1:sizes[j-1,2]] )
                            }

                            ## Add back the ignore columns
                            data_resample <- X1[return_idxs,
                                                !(colnames(X1) %in% ignore.columns)]
                            if(!is.null(ignore.columns)){
                              if(nrow(data_resample)==nrow(data)){
                                data_resample <- cbind(data[ignore.columns], data_resample)
                              }else{
                                tmp <- matrix(NA, ncol=length(ignore.columns),
                                  ...
                                  if(sum(idxs)!=sum(idxs_orig)){
                                    if(sum(idxs)>sum(idxs_orig)){
                                      ...
                                      add_dat <- data[idxs_orig,
                                                      ...
                                                      drop=FALSE]
                                      add_dat <- add_dat[
                                        sample(1:sum(idxs_orig),size=sum(idxs),
